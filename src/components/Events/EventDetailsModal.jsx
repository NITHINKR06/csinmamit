...
const EventDetailsModal = ({ event, isOpen, onClose }) => {
  ...
  const handleCreateTeam = async () => {
    ...
    try {
      ...
      if (error) {
        if (typeof error === 'object') message = error.message || error.code || message
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      try { await navigator.share({ title: event.title, text: `Check out this event: ${event.title}`, url: shareUrl }); toast.success('Event shared!') } catch (error) { if (error.name !== 'AbortError') copyToClipboard(shareUrl) }
    } else { copyToClipboard(shareUrl) }
  }

  const copyToClipboard = (text) => { navigator.clipboard.writeText(text); toast.success('Event link copied to clipboard!') }
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="rounded-xl p-5 bg-gray-50 dark:bg-gray-800 ring-1 ring-black/5 dark:ring-white/5">
                <div className="grid grid-cols-1 gap-4">
                  {[
                    { icon: Calendar, color: 'text-primary-500', label: 'Date', value: formatEventDate(event.date) },
                        ...
                        <div>
                          ...
                          <select value={teamSize} onChange={(e) => setTeamSize(parseInt(e.target.value))} className="input-field">
                            {Array.isArray(event.teamSizeOptions) && event.teamSizeOptions.length
                              ? event.teamSizeOptions.sort((a,b)=>a-b).map(sz => (<option key={sz} value={sz}>{sz} {sz === 1 ? 'member' : 'members'}</option>))
